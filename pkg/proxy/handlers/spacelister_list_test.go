...
func communityTests(t *testing.T, fakeClient *test.FakeClient) map[string]testData {
	return map[string]testData{
		"no spaces found for current user": {
...
func noCommunityTests(t *testing.T, fakeClient *test.FakeClient) map[string]testData {
	return map[string]testData{
		"no spaces found for current user": {
			username:        "user.nospace",
			expectedWs:      []toolchainv1alpha1.Workspace{},
...
func sharedTests(t *testing.T, fakeClient *test.FakeClient) map[string]testData {
	return map[string]testData{
...
func TestSpaceListerList(t *testing.T) {
	...
	t.Run("HandleSpaceListRequest", func(t *testing.T) {
		// given
		runTests := func(fakeSignupService *fake.SignupService, fakeClient *test.FakeClient, tests map[string]testData) {
		...
		t.Run("shared tests", func(t *testing.T) {
			t.Run("shared: no community", func(t *testing.T) {
				fakeSignupService, fakeClient := buildSpaceListerFakes(t, false)
				runTests(fakeSignupService, fakeClient, sharedTests(t, fakeClient))
			})
			t.Run("shared: community", func(t *testing.T) {
				fakeSignupService, fakeClient := buildSpaceListerFakes(t, true)
				runTests(fakeSignupService, fakeClient, sharedTests(t, fakeClient))
			})
		})

		t.Run("no community", func(t *testing.T) {
			fakeSignupService, fakeClient := buildSpaceListerFakes(t, false)
			runTests(fakeSignupService, fakeClient, noCommunityTests(t, fakeClient))
		})
		t.Run("community", func(t *testing.T) {
			fakeSignupService, fakeClient := buildSpaceListerFakes(t, true)
			runTests(fakeSignupService, fakeClient, communityTests(t, fakeClient))
		})
	})
}
